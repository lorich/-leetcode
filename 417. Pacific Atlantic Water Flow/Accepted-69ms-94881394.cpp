...
public:
	...
	vector<pair<int, int>> pacificAtlantic(vector<vector<int>>& matrix) {
		...
		visited.resize(m, vector<char>(n, 0));
		for (int i = 0; i < m; i++) {
			dfs(matrix, i, 0, 0, 1);
			dfs(matrix, i, n - 1, 0, 2);
		}
		for (int i = 0; i < n; i++) {
			dfs(matrix, 0, i, 0, 1);
			dfs(matrix, m - 1, i, 0, 2);
		}
		return res;
	}
};
...
[[0, 4], [1, 3], [1, 4], [2, 2], [3, 0], [3, 1], [4, 0]] (positions with parentheses in above matrix).
 Subscribe to see which companies asked this question. Show Tags Depth-first Search Breadth-first Search
*/
