...
public:
	vector<int> findRightInterval(vector<Interval>& intervals) {
		map<int, int> mStartIndex;
		for(int i=0; i<intervals.size(); ++i) {
			mStartIndex[intervals[i].start] = i;
		}
		vector<int> vRes;
		for(auto ele:intervals) {
			auto itr = mStartIndex.lower_bound(ele.end);
			vRes.push_back(itr == mStartIndex.end()?-1:itr->second);
		}
		return vRes;
	}
};
...
For [2,3], the interval [3,4] has minimum-"right" start point.
 Subscribe to see which companies asked this question. Show Tags Binary Search Show Similar Problems (H) Data Stream as Disjoint Intervals
*/
